...
const CATEGORIES = [
  { nom: 'Plomberie', icone: '🔧', couleur: '#2563eb' },
  { nom: 'Électricité', icone: '⚡', couleur: '#d97706' },
  { nom: 'Serrurerie', icone: '🔑', couleur: '#7c3aed' },
  { nom: 'Propreté', icone: '🧹', couleur: '#16a34a' },
  { nom: 'Autre', icone: '📋', couleur: '#64748b' },
];
...
const PROBLEMES_PAR_ZONE = {
  ...
  'Douches': ["Fuite d'eau", "Panne d'eau chaude", 'Carrelage cassé', 'Mauvaise odeur / moisissure'],
          ...
          {etape === 1 && !zone && (
            <motion.div key="zones" variants={variantes} initial="entree" animate="centre" exit="sortie" transition={{ duration: 0.25 }}>
              ...
              <div className="signalement-grille">
                {ZONES.map((z) => (
                  <button key={z} className="signalement-choix" onClick={() => setZone(z)}>{z}</button>
                ))}
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="problemes" variants={variantes} initial="entree" animate="centre" exit="sortie" transition={{ duration: 0.25 }}>
              ...
              <div className="signalement-liste-problemes">
                {PROBLEMES_PAR_ZONE[zone].map((p) => (
                  <button key={p} className="signalement-probleme" onClick={() => choisirProbleme(p)}>{p}</button>
                ))}
            ...
            <motion.div key="categorie" variants={variantes} initial="entree" animate="centre" exit="sortie" transition={{ duration: 0.25 }}>
              ...
              <div className="signalement-grille">
                {CATEGORIES.map((c) => (
                  <button
                    key={c.nom}
                    className={`signalement-choix ${categorie === c.nom ? 'selectionne' : ''}`}
                    style={{ '--couleur-categorie': c.couleur }}
                    onClick={() => { setCategorie(c.nom); etapeSuivante(); }}
                  >
                    <span className="signalement-choix-icone">{c.icone}</span>
                    {c.nom}
                  </button>
                ))}
              </div>
          ...
          {etape === 4 && (
            <motion.div key="recap" variants={variantes} initial="entree" animate="centre" exit="sortie" transition={{ duration: 0.25 }}>
              ...
              <div className="signalement-recap">
                ...
                <span
                  className="signalement-tag"
                  style={{
                    background: `${CATEGORIES.find(c => c.nom === categorie)?.couleur}22`,
                    color: CATEGORIES.find(c => c.nom === categorie)?.couleur,
                  }}
                >
                  {categorie}
                </span>
